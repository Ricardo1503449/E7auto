...
def build_manifest(source_paths: dict[str, Path] | None = None) -> dict[str, object]:
    # Historical defaults support replay of the recorded evidence. The CLI
    # always supplies explicit paths, without rewriting SOURCE_SPECS globally.
    specs = SOURCE_SPECS
    if source_paths is not None:
        if set(source_paths) != set(SOURCE_SPECS):
            raise ValueError(f"Source roles must be exactly {sorted(SOURCE_SPECS)}")
        specs = {
            role: {**spec, "path": source_paths[role].resolve()}
...
def main() -> int:
    ...
    for role in SOURCE_SPECS:
        parser.add_argument("--" + role.replace("_", "-") + "-source", type=Path, required=True)
    parser.add_argument(
        "--output",
        type=Path,
