...
当前检入源码的配置已针对经过验证的目标主机完成全部校准，并设置了 `calibration_complete: true`。当该校准门控、必要模板或安全关键几何信息缺失时，配置加载仍会安全失败，并且不会发送任何输入。当前独立发布版为 `v1.1.4`，对应压缩包为 `E7auto_v1.1.4_x64.zip`，请从 [GitHub Releases](https://github.com/Ricardo1503449/E7auto/releases/latest) 下载。

目前已经记录以下配置：可执行文件名 `EpicSeven.exe`、精确窗口标题 `第七史诗`、物理客户区校准基准 `2322 x 1306`、参考桌面模式 `3120 x 2080`、最低桌面模式 `2560 x 1440`、刷新费用、模板、完整的天空石数字 `0-9`、进入/退出/刷新/对话框/天空石/商品列表/金币不足的 ROI 与点击点、六个互不重叠的商品槽位、向下滚动序列（`-120` 重复 6 次）、空列表稳定扫描时序（`100 ms`、3 帧、3000 ms 超时）、经操作者确认的 `18 px` 悬浮窗偏移 `(-252,-145)`、悬浮窗截图排除，以及前台实时 `购买金币` 识别。程序通过精确窗口标题和可执行文件名定位正在运行的游戏，因此无需配置游戏安装目录。终止条件检查在不发送输入且不保存截图的情况下，以 `0.9999991492` 的置信度连续通过 5/5 帧。现有四项等待上限均已由用户审核并保持不变。设计中不使用固定的顶部/底部导航锚点。购买成功状态根据匹配到的商品槽位动态识别，不依赖相对于整个窗口的绝对结果 ROI。

窗口策略使用游戏所在显示器的当前有效桌面模式（物理像素），并要求它与 Win32 完整显示器矩形一致。仅 `3120 x 2080` 使用原始 `2322 x 1306` 零缩放路径；其他受支持模式先取完整显示器宽度的 60% 作为客户区宽度，再按基准宽高比计算高度，并在 DPI 感知的完整外框超过显示器高度时等比缩小。宽度低于 `2560` 或高度低于 `1440`、查询失败、运行中迁移显示器/改变分辨率或 DPI 时均在下一次输入前安全停止。非参考路径只在内存中按需裁剪并归一化基准 ROI，不生成缩放模板或完整缩放帧。
...
- 所有识别 ROI、锚点、点击点、滚轮位置和首次悬浮窗偏移都从唯一的 `2322 x 1306` 校准空间映射到实际客户区；输入前重新核对显示器、当前模式和 DPI，运行中不会自动二次缩放或改用新的坐标基准。
...
构建脚本会嵌入 `requireAdministrator` UAC 清单。`dist` 仅保存本机构建结果，不纳入 Git 跟踪；通过发布检查的版本化 ZIP 应作为 GitHub Release 附件上传。
