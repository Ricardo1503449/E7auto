...
def test_background_validator_has_no_image_writer_or_focus_mutation() -> None:
    ...
    guide = (ROOT / "docs" / "validation" / "platform" / "BACKGROUND_VALIDATION.md").read_text(encoding="utf-8")
    assert "-Mode capture" in guide
    assert "scroll --acknowledge-shop-top-covered --capture-backend wgc --interval-ms 10" in guide
    assert "-Mode navigation" in guide
    assert "--effect-observation-ms 10000" in guide
