...
def test_tool_paths_resolve_outside_project_cwd(tmp_path: Path) -> None:
    code = (
        ...
        "from scripts.common.paths import PROJECT_ROOT, TEMPLATES_DIR, CALIBRATION_DIR, calibration_record_path; "
        "assert PROJECT_ROOT == Path(sys.argv[1]); "
        "assert (calibration_record_path('manifest.yaml')).is_file()"
    )
    result = subprocess.run(
