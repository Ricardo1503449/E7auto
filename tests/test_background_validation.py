...
from scripts import validate_background_mode
...
def test_background_validator_has_no_image_writer_or_focus_mutation() -> None:
    source = (ROOT / "scripts" / "validate_background_mode.py").read_text(encoding="utf-8")
    for forbidden in (
        "cv2.imwrite",
        "Image.save",
        "SetForegroundWindow",
    ...
    wrapper = (ROOT / "scripts" / "run-admin-background-validation.ps1").read_text(
        encoding="utf-8"
    )
