...
- 来源及补登记细节见[模板工作流验证](TEMPLATE_WORKFLOW_VALIDATION.md)和[正式来源记录](calibration/registrations/shop-sky_stone_icon-c78eaf04b03d-424d239c56ae.json)。
...
- 入口识别：[入口识别验证](ENTRY_RECOGNITION_VALIDATION.md)。
- 单次启动唤醒：[启动唤醒验证](STARTUP_WAKE_VALIDATION.md)。
- 模板组织与登记：[模板工作流验证](TEMPLATE_WORKFLOW_VALIDATION.md)。
- 本版还包含滚动重叠校验兜底、异常停止缓存帧诊断及日志数值格式整理，详见更新日志。

### 本轮验证

- 已同步 `pyproject.toml`、`src/e7auto/__init__.py`、README、使用说明与本机 editable 安装元数据至 1.3.2。
- `.venv/Scripts/python.exe -m pytest tests/scripts/test_version_sync.py tests/test_template_assets.py tests/test_template_layout.py tests/test_template_repository.py tests/ui/test_assets.py -q`：50 passed in 5.14s。只运行版本、资源布局、模板读取及 UI 打包资源相关检查，未运行全量套件。
- `.venv/Scripts/python.exe -m scripts.verify_environment`：通过，project/source/installed 均为 1.3.2，锁定依赖无缺失或版本不匹配。
- `git diff --check`：通过。
- 原始输出：`artifacts/release-v1.3.2/focused-tests.txt`、`environment.json`。
...
- 原始输出：`artifacts/release-v1.3.2/build.log`、`build.exit`、`archive-verification.json`；校验脚本为同目录 `verify_archive.py`。
...
- 输出：`artifacts/release-v1.3.2/release-verification.txt`、`release-verification.exit`；管理员入口为同目录 `verify-release-admin.ps1`。
