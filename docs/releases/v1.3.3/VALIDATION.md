...
日期：2026-09-16。当前为源码准备状态，尚未构建v1.3.3 EXE/ZIP、执行管理员编译版自检、创建标签或发布GitHub Release。现有dist仍为v1.3.2，不能用于判断本次修复是否生效。
...
`pyproject.toml`、`src/e7auto/__init__.py`、README、CHANGELOG、使用说明及发布检查的当前版本统一为1.3.3；既有版本历史记录保留。项目editable安装通过本地无网络、无依赖更新的重新安装同步。目标包名为 `E7auto_v1.3.3_x64.zip`，Windows文件/产品版本由构建脚本读取项目版本生成 `1.3.3.0`，尚未生成实际二进制。
...
纠正后布局/源码依赖边界检查与 `git diff --check` 通过，31个相关未提交文件均已核对并分配至提交方案；无关pip.ini保留。完整结果见 `layout-final.log`、`diff-check.log`、`final-verification.json`，提交范围与message见 `commit-proposal.md`。本轮只准备审核材料，尚未暂存、提交或推送。
...
后续经授权构建后，按[发布检查](../../development/RELEASE_CHECKLIST.md)验证DLL一致性、资源/ZIP完整性、Windows版本和管理员编译自检；真实游戏与纯净Windows验证仍需单独完成。源码测试和构建脚本契约通过，不代表这些发行验收已通过。
