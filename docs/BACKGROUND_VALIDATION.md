...
```powershell
powershell -ExecutionPolicy Bypass -File scripts\validation\run-admin-background-validation.ps1 -Mode scroll -CaptureBackend wgc -EffectObservationMs 10000
```
...
```powershell
powershell -ExecutionPolicy Bypass -File scripts\validation\run-admin-background-validation.ps1 -Mode scroll
```
